...
- [ ] **Step 3: Migrare eventi, guest e RSVP** — **metà fatto**. Eventi completi (lista viva, evento singolo vivo via `useEvent`, create/update/delete a mutation) e statistiche vive (`api.events.stats`: il **polling a 30s è stato rimosso**). Editor, configurazione RSVP e distribuzione leggono l'evento **una volta** (`useEventActions().getEventOnce`) perché lo copiano in stato editabile: una query viva lì riscriverebbe il form sotto le dita dell'utente. **Non fatti:** `useEventGuests.ts` e `usePublicInvite.ts`, e non è solo ricablaggio — il tipo di job `send-invite-email` ha un consumatore ma **nessun produttore** in Convex (i produttori esistenti sono i cron), quindi servono `guests.sendInvites` e l'equivalente di `send-test`.
...
- [ ] **Step 4: Migrare organizzazione e billing** — **non iniziato.** Nota per chi lo fa: `organizationStore` non usa `$fetch` ma il **client plugin di Better Auth** (`client.organization.*`), quindi il gate del Step 1 non lo vedrà mai; le funzioni Convex necessarie esistono già tutte (`organizations.*`, `billing.checkoutsCreate`/`customersPortalUrl`/`planForActiveOrganization`).
...
- [ ] **Step 5: Migrare profilo, export e form pubblici** — **non iniziato.** I tre bridge anonimi esistono già (Task 12); restano `profileStore`, i due componenti di export e gli upload (presign Convex → PUT su R2 → confirm). Restano invece **Worker per vincolo di trasporto**, non per debito: `GET /api/events/:id/export` (CSV) e `/guests/:id/qr` (PNG) sono download binari, `/api/user/data-export/download/:token` è un URL firmato; `feedbackStore` (`/api/suggestions*`) è fuori dal piano.
