...
- **Task 15:** **code-complete ermeticamente** (2026-09-24), verifica browser **non eseguita**. Console `/admin` su Convex (`convex/admin.ts`, 13 query + 4 mutation pubbliche + bootstrap `internal`), sessione Better Auth e ruolo `appUsers.globalRole`, non API key; la console legacy `server/api/admin/**` resta intatta (rimozione Task 19). `requireSuperAdmin` è la prima istruzione di ogni export (e ora anche di `jobs.retryDead`/`siteSettings.set/clear`); ogni write richiede una motivazione non vuota, è rate limited sul bucket `admin` e scrive audit con attore/target/timestamp/`details.reason`/dettagli nella stessa transazione; nessuna lettura restituisce hash, token, URL/chiavi di export, payload dei job o metadata Creem (`redactSecrets` sui dettagli audit). Bootstrap del primo superAdmin via `SUPER_ADMIN_EMAIL_ALLOWLIST`, rifiutato quando un superAdmin esiste già. I "limiti custom" legacy non esistevano più: nuova tabella `organizationLimitOverrides` (per org) letta da ogni punto di enforcement. 23 casi in `convex/admin.test.ts` (matrice anonimo/utente/superAdmin su **tutti** gli export, con un controllo di completezza sul modulo). La spec Playwright (`test/migration/admin-console-live.test.ts`, `pnpm test:e2e:admin`) è scritta ma **non eseguita**: nessuna credenziale di staging qui — spostata al rehearsal del Task 16. Limite noto: in `maintenance` le pagine `/admin` non sono raggiungibili (esentato solo `/api/admin/*`). Dettagli: `docs/migration/admin-console.md`.
- **Task 16–18:** non avviati. Runtime, dati applicativi di dominio, billing e le restanti superfici frontend restano su Neon/Drizzle; la configurazione locale usa `NUXT_NITRO_PRESET=node-server`.
...
- [x] **Step 3: Implementare dashboard e ricerca indicizzate** — contatori con tetti dichiarati (`METRIC_CAPS`); subscription in sola lettura (nessuna azione di scrittura verso Creem); limiti custom su `organizationLimitOverrides` (tabella nuova).
