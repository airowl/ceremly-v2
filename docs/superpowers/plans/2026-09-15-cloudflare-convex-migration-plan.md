...
- **Final review Task 14–17 — fix (2026-09-25):** Critical e Important chiusi ermeticamente, nessun sistema esterno toccato, nulla pushato. **C1:** il blu non può eseguire nessun commit da `e6bfe5d` (frontend solo-Convex): nuovo branch locale **`legacy-vercel`** (`7c01069` + `c4c0b56`, solo il port lato server della read-only del Task 17, nessun break-glass admin, audit/self-heal soppressi anche in `maintenance`), `NUXT_NITRO_PRESET=vercel pnpm build` exit 0 e 147 test di migrazione verdi su quel branch; `cutover.md`/`rollback.md` costruiscono Vercel produzione e il tag `legacy-vercel-final` da `legacy-vercel` e Worker/Convex da `main` HEAD, con passo 0.0 dell'operatore sul Production Branch di Vercel (non verificabile da qui) e l'avviso che `origin/main` contiene già `e6bfe5d`; il preflight (`deploymentIds`) controlla ciascuno stack contro il proprio commit (`legacyBuiltFromCommit` = tag, non discende da `e6bfe5d`, discende dal port; fail-closed se git non risponde). **C2:** cron e job Convex girano solo in `active` (`sideEffectsAllowed`): ogni cron è un no-op loggato, `jobs.run` lascia il job `pending`/`retrying` senza consumare tentativi, test enumerativo su `crons.ts` e `JOB_TYPES` (`convex/siteModeSideEffects.test.ts`). **I1:** `deleteOrganization` cancella a cascata tramite il nuovo job durevole `organization-purge` (nono tipo; R2 prima delle righe, lotti limitati, audit `organization.purged`), e da subito l'invito pubblico è 404 e nessun reminder/email parte per un'organizzazione cancellata. **I2:** break-glass per path esatti e simmetrico fra Worker e `.convex.site` (backup code ammesso in `maintenance`/`waitinglist`, mai 2FA enable/disable, OAuth, sign-up). Minori: M1 limiti Zod degli ospiti, M2 bucket `emailSend`, M3 doc della console, M4 webhook Resend diretto `503` fuori dalle modalità scrivibili, M6 messaggio leggibile per `SITE_READ_ONLY`, M7 segreti fuori dalla riga di comando del runbook. Verifica: `pnpm test` 891 passati (29 skip live), `pnpm vitest run convex/` 416, `pnpm typecheck` 23 errori (baseline, nessuno nuovo), `typecheck:convex` e build verdi. **Aperto, da decidere:** i Vercel Cron del blu in `maintenance` dopo il passo 10 (vedi `cutover.md` §10). Report: `.superpowers/sdd/2026-09-15-cloudflare-convex-migration-plan/final-fixes-migration-report.md`.
