 ...
 */
// Type-only import from outside `convex/`: erased by the bundler (`import type`
// + isolatedModules), so no shared runtime code ever enters a Convex function.
// The module moved here in Task 11 so that Convex *is* the source of truth for
// the templates it expands at event creation — a mirror would have been a second
// copy to keep in sync with the editor UI, which is exactly the drift this
// avoids. `shared/constants/templates.ts` re-exports it for the client.
import type { EventDistribution, EventTypeKey, InviteBlock } from "../../shared/types/ceremly";
